...
df_motorbike = read.table(file = "../Desktop/Data Science/R/datasets/Accidents 5 years/motorbikes.csv",
                     sep = ",",
                     header = TRUE)
...
# --- engine capacity ---------
# cars engine
engine <- df_motorbike$engine_capacity_cc
engine
str(engine)
...
hist(engine, probability = TRUE,xlab = "Engine capaity cc", ylab = "", col = "grey",
     axes = FALSE, main = "Histogram, density function and boxplot", breaks = 80)
...
# Cleaning outliers
engine <- engine[(engine>=0) & (engine<=1402)]
...
engine <- engine[(engine>=42) & (engine<=1402)]
# Third Graphic
hist(engine, probability = TRUE,xlab = "Engine capaity cc", ylab = "", col = "grey",
...
stats_engine
# Without outliers
engine_Frequency = table(engine)
sort(engine_Frequency, decreasing = TRUE)

engine_prop = prop.table(engine_Frequency) * 100
engine_prop = engine_prop[engine_prop>0.5]
barplot(engine_prop, xlab = "Engine capacity", ylab = "Proportion %", main = "Proportion of accidents by Engine", col = "lightblue")
engine_prop
...
# --------- Cars: Age of driver
# age of driver
age_driver <- df_motorbike$age_of_driver
...
# cleaning data
age_driver <- age_driver[(age_driver>=18)&(age_driver<=77)]
str(age_driver)
...
# Without outliers
age_driver_frequency = table(age_driver)
sort(age_driver_frequency, decreasing = TRUE)

age_driver_prop = prop.table(age_driver_frequency) * 100

barplot(age_driver_prop, xlab = "Moto: Age drriver", ylab = "Proportion %", main = "Proportion of accidents by Engine", col = "lightblue")
age_driver_prop
...
# Sex of driver
sex_driver <- df_motorbike$sex_of_driver
...
model <- df_motorbike$generic_make_model
model
model <- model[model!=-1]
...
# -------- Age of vehicle ----------------
age_vehicle = df_motorbike$age_of_vehicle
...
# cleaning data
age_vehicle <- age_vehicle[(age_vehicle>=0)&(age_vehicle<=22)]
str(age_driver)
